...
@maybe_decorate(nb.njit(nb.f8[:](nb.f8[:], nb.f8[:])), nb is not None)
...
@maybe_decorate(nb.njit(nb.types.f8[:](nb.types.f8[:])), nb is not None)
...
def generate_field_fft(shape, sd=(0.33, 0.33, 0.34), len_sc=(0.5, 0.5 / 4, 0.5 / 16)):
    from featstat.algo.image import ImageProc
...
@maybe_decorate(nb.njit(nb.types.f8[:](nb.types.f8[:], nb.types.f8[:], nb.types.f8[:])), nb is not None)
...
def q_times_img_coords(q, pts2d, cam, distort=True):
    # project to unit sphere
    vo = cam.to_unit_sphere(pts2d, undistort=distort)
    ...
    pt2d_n[mask, :] = cam.calc_img_R(vn[mask, :], distort=distort) - 0.5
    return pt2d_n
